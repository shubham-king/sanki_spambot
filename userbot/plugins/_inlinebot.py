...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
        ...
        if query.startswith("**Catuserbot") and event.query.user_id == bot.uid:
            buttons = [
                (
                    ...
                    Button.url("Repo", "https://github.com/shubham-king/sanki_spambot"),
                )
            ]
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:  # pylint:disable=E0602
            ...
            reply_pop_up_alert = "Please get your own Sanki userbot, and don't use mine! Join @RobotTech_official help"
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:  # pylint:disable=E0602
            ...
            reply_pop_up_alert = "Please get your own Sanki useebot , and don't use mine! Join @RobotTech_official help "
            await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            ...
            reply_pop_up_alert += (
                "Use .unload {} to remove this plugin © Sanki spammers".format(plugin_name)
            )
            try:
                await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
            except BaseException:
